...
class TricountAPI():
    ...
    def __generate_installation_id(self, app_id:str) -> str:
        ...
        # Warning message
        warnings.warn(f'No `app_id` provided. Generated new app ID: {app_installation_id}. For best practice, pass a consistent `app_id` when creating the TricountAPI instance.', UserWarning, stacklevel=3)
    ...
    def __requests_json(self) -> dict:
        ...
        tricount_data = self.session.get(f'{self.base_url}/v1/user/{user_id}/registry?public_identifier_token={self.tricount_key}')
